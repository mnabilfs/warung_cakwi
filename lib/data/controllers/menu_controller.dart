...
class MenuController extends GetxController {
  ...
  Future<void> _initializeBoxes() async {
    try {
      ...
      // Delay untuk memastikan AuthController sudah ready
  ...
  void _checkAuthAndSync() async {
    if (Get.isRegistered<AuthController>()) {
      ...
      if (authC.isLoggedIn) {
        bool hasOfflineChanges = settingsBox.get(
          'has_offline_changes',
          defaultValue: false,
        );
        ...
        if (hasOfflineChanges) {
          print(
            "🔄 Mendeteksi perubahan offline. Menimpa Cloud dengan data Lokal...",
          );
  ...
  void removeFromCart(int index) async {
    ...
    final confirmed = await Get.dialog<bool>(
      AlertDialog(
        ...
        content: Text(
          'Yakin ingin menghapus "${cartItems[index].name}" dari keranjang?',
        ),
        actions: [
          TextButton(
